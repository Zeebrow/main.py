...
class AWSHost:
    ...
    def get_hosts_for_app(self, app_name: str):
        ...
        store_test_data(resource='AWSHost', action='describe', response_data=convert_datetime_to_string(_app_hosts))
    ...
    def wait_for_hosts(self):
        ...
        while True:
            ...
            _running_hosts = []
            _other_hosts = []
            _app_hosts = convert_datetime_to_string(self.client.describe_instances(
                Filters=[
                    { 'Name': f"tag:{DEFAULT_APP_NAME}", 'Values': [ self.app_name, ] },
